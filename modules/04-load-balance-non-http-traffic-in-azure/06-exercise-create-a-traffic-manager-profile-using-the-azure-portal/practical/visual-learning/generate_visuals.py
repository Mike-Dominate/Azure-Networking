...
def arrow(draw, p1, p2, width=5):
    ...
    pts = [
        p2,
        (p2[0] - length * math.cos(angle - 0.5), p2[1] - length * math.sin(angle - 0.5)),
        (p2[0] - length * math.cos(angle + 0.5), p2[1] - length * math.sin(angle + 0.5)),
    ]
...
def save(img, name):
    # Palette quantization keeps the repo-friendly PNGs small without losing diagram clarity.
    img.quantize(colors=64, method=Image.Quantize.MEDIANCUT).save(OUT / name, optimize=True)
...
def visual_01():
    ...
    box(d, (890, 180, 1490, 420), "Azure Traffic Manager", [
        "tm-az700-global",
    ...
    box(d, (1010, 570, 1490, 790), "Selected regional ACI endpoint", [
        "Example for GEO-AP:",
...
def visual_02():
    ...
    box(d, (560, 135, 1040, 300), "Traffic Manager", [
        "Geographic routing",
        "Decision is based on DNS query / resolver geography",
    ])
    regions = [
...
def visual_04():
    ...
    box(d, (500, 150, 1100, 365), "Azure Traffic Manager — tm-az700-global", [
        "FQDN: az700-tm-md-87004.trafficmanager.net",
        "Routing: Geographic | DNS TTL: 30s",
        "Monitor: HTTP :80 / | interval 30s | failures 3 | timeout 10s",
    ])
    endpoints = [
    ...
    for x1, y1, x2, y2, region, mapping, container, fqdn in endpoints:
        box(d, (x1, y1, x2, y2), region, [mapping, container, fqdn, "Public HTTP :80", "Traffic Manager target = FQDN"])
